...
### 6. 自定义git
#### 6.1 忽略特殊文件
&emsp;有些时候，你必须把某些文件放到Git工作目录中，但又不能提交它们，比如保存了数据库密码的配置文件,中间文件以及目标文件等，这些文件不但占得内存比较大，而且非常多。在Git工作区的根目录下创建一个特殊的.gitignore文件，然后把要忽略的文件名填进去，Git就会自动忽略这些文件。使用git status的时候就不需要面对太多无效信息。</br>
&emsp;git已经给我们列出了各种配置文件，我们只需要组合这些就可以了<https://github.com/github/gitignore>。</br>
#### 6.2 配置别名
#### 6.3 搭建git服务器
